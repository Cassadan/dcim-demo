...
(function (global, factory) {
    ...
    function isVisible(element) {
        return toNodes(element).some(
            (element2) =>
                element2.offsetWidth || element2.offsetHeight || element2.getClientRects().length
        );
    }
    ...
    const splitSelector = memoize((selector) =>
        selector.match(selectorRe).map((selector2) => selector2.replace(/,$/, "").trim())
    );
        ...
        each(css(element, ["left", "top"]), (value, prop) =>
            css(
                ...
                coordinates[prop] -
                    currentOffset[prop] +
                    toFloat(
                        pos === "absolute" && value === "auto" ? position(element)[prop] : value
                    )
            )
        );
    }
        ...
        return {
            ...
            handler(e) {
                ...
                once(document, `${pointerUp$1} ${pointerCancel} scroll`, (e2) => {
                    ...
                    if (
                        (e2.type !== "scroll" && target && x && Math.abs(pos.x - x) > 100) ||
    ...
    var Dropnav = {
            ...
            },
            {
                ...
                handler(e) {
                    ...
                    if (
                        ...
                        active2.$el === e.target &&
                        !this.items.some((el) => active2.targetEl !== el && matches(el, ":focus"))
                    ) {
                        e.preventDefault();
                    }
                },
        ...
        methods: {
            getActive() {
                var _a;
                return (
                    includes(this.dropdowns, (_a = active$1) == null ? void 0 : _a.$el) && active$1
                );
            },
    ...
    function getHeight(element) {
        ...
        const height =
            dimensions$1(element).height - boxModelAdjust(element, "height", "content-box");
        css(element, style);
        return height;
    }
        ...
        update: [
            {
                read({ height: height$1, width, margin, sticky }, types) {
                    ...
                    const overflow = this.overflowFlip
                        ? 0
                        : Math.max(0, height$1 + offset$1 - viewport);
                    ...
                    if (
                        ...
                        (this.showOnUp &&
                            (scroll2 <= start ||
                                ...
                                (dir === "up" && !this.isFixed && scroll2 <= topOffset + height)))
                    ) {
                        if (!this.isFixed) {
    ...
    var svg = {
        ...
        methods: {
            async getSvg() {
                ...
                return (
                    parseSVG(await loadSVG(this.src), this.icon) || Promise.reject("SVG not found.")
                );
            },
        },
    };
    ...
    var SliderDrag = {
        ...
        methods: {
            ...
            move(e) {
                ...
                let width =
                    this._getDistance(prevIndex, nextIndex) || slides[prevIndex].offsetWidth;
    ...
    function hasSelectableText(el) {
        return (
            ...
            toArray(el.childNodes).some((el2) => el2.nodeType === 3 && el2.textContent.trim())
        );
    }
        ...
        created() {
            ...
            this.$mount(
                append(
                    ...
                    }" role="alert"> <a href class="${this.clsClose}" data-uk-close></a> <div>${
                        this.message
                    }</div> </div>`
                )
            );
    ...
    function backgroundCoverFn(prop, el, stops, props2) {
        ...
        for (const prop2 of bgProps) {
            ...
            positions[prop2] = `max(${getBackgroundPos(el, prop2)},-${overflow}px) + ${
                positions[prop2]
            }`;
        }
    ...
    function getStop(stops, percent) {
        const index =
            findIndex(stops.slice(1), ([, targetPercent]) => percent <= targetPercent) + 1;
        return [
            stops[index - 1][0],
            stops[index][0],
